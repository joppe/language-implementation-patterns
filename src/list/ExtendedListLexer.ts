...
/**
 * @class ExtendedListLexer
 */
export class ExtendedListLexer extends LL1Lexer {
    /**
     * @returns {Token}
     * @throws {Error}
     */
    public nextToken(): Token {
        while (this.char !== EOF) {
            ...
            } else if (EQUALS === this.char) {
                this.consume();

                return new Token(TokenTypes.EQAULS, this.getTokenName(TokenTypes.EQAULS), EQUALS);
            } else {
                if (this.isLetter()) {
                    return this.name();
                } else {
